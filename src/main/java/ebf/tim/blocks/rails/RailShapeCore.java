...
public class RailShapeCore {
    ...
    public Vec6f start;
    public Vec6f center=new Vec6f(0,0,0,0,0,0);
    public Vec6f end;
    /*CLIENT ONLY*/
    ...
    public static RailShapeCore multiTriGenModel(RailSimpleShape shape, int[] railOffsets){
        ...
        sc.start=shape.getStart();
        sc.center=shape.getCenter();
        sc.end=shape.getEnd();
        sc.tieCount=shape.getSleeperCount();
        List<Vec6f> points;
        float t;
        int i;
        ...
        for (i=1; i < points.size() - 1; i++) {
            //be sure this doesn't fire off if we're on the first or last point and rotation calculation for it is disabled
