...
public class Model1x1Rail {
    ...
    public static void Model3DRail(World world, int xPos, int yPos, int zPos, RailShapeCore shape){
        ...
        switch (ClientProxy.railSkin){
            case 0:{
                ModelRail.modelPotatoRail(shape, shape.renderScale, colors); break;}
            case 1:{
                ModelRail.modelExtrudedRail(shape, shape.renderScale, colors); break;}
            case 2:{
                ModelRail.model3DRail(shape, shape.renderScale, colors); break;}
            case 3:{
                ModelRail.modelHDRail(shape, shape.renderScale, colors); break;}
        }
        ...
        if(shape.ballast!=null && shape.ballast.getItem()!=null) {
            ...
            if(ClientProxy.railSkin==0){
                ...
                ModelBallast.modelHDBallast(shape,  maxWidth, minWidth, shape.renderScale, shape.ballast);
            } else {
        ...
        if(shape.ties!=null && shape.ties.getItem()!=null) {
            GL11.glPushMatrix();
            if(ClientProxy.railSkin==0){
                ModelTies.modelPotatoTies(shape,  maxWidth, minWidth, shape.renderScale, shape.ties);
            } if (ClientProxy.railSkin==3) {
                ModelTies.modelHDTies(shape, maxWidth, minWidth, shape.renderScale, shape.ties, colors);
            } else {
                ModelTies.model3DTies(shape, maxWidth, minWidth, shape.renderScale, shape.ties);
            }
            GL11.glPopMatrix();
        }
