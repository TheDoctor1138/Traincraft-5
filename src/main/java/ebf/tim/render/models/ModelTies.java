...
public class ModelTies {
    ...
    public static List<Vec5f> genTiePoints(RailShapeCore shape){
        if(shape==null){
            return new ArrayList<>();
        }
        float originalT=1f/shape.tieCount;//1/6
        ...
        while (t <= 1+(originalT*0.51f)) {
            ...
            points.add(new Vec5f(
                    (((1f - t) * (1f - t)) * shape.start.xCoord) + (2f * (1f - t) * t * shape.center.xCoord) + ((t * t) * shape.end.xCoord),//X
                    (((1f - t) * (1f - t)) * shape.start.yCoord) + (2f * (1f - t) * t * shape.center.yCoord) + ((t * t) * shape.end.yCoord),//Y
                    (((1f - t) * (1f - t)) * shape.start.zCoord) + (2f * (1f - t) * t * shape.center.zCoord) + ((t * t) * shape.end.zCoord)//X
                    ,0,0));
            t += originalT;
        }
        //define rotations
    ...
    public static List<Vec5f> modelPotatoTies(RailShapeCore shape, float maxWidth, float minWidth, float scale, ItemStack block){
    ...
    public static void model3DTies(RailShapeCore shape, float maxWidth, float minWidth, float scale, ItemStack block){
    ...
    public static void modelHDTies(RailShapeCore shape, float maxWidth, float minWidth, float scale, ItemStack block, int[] color){
        maxWidth += 0.03125f;
        minWidth -= 0.03125f;
