...
public class GuiCrafterTier extends GuiTraincraft {
    ...
    private static final int[] states = new int[10];
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
        ...
        if (currentKnownItem != null && isTabRecipeOpen) {
            ...
            for (int a = 0; a < 6; a++) {
                for (int b = 0; b < 5; b++) {
                    // Black Squares
                    drawTexturedModalRect(j - 120 + a * 16, (k + currentRenderTabY + b * 16), 240, 48, 16, 16);

                    // Border Left
                    drawTexturedModalRect(j - 120 - 1, (k + currentRenderTabY + b * 16), 252, 68, 2, 16);
                    // Border Right
                    drawTexturedModalRect(j - 120 - 1 + 6 * 16, (k + currentRenderTabY + b * 16), 254, 68, 2, 16);
                    // Border Bottom
                    drawTexturedModalRect(j - 120 + a * 16, (k + (currentRenderTabY - 1) + 5 * 16), 240, 64, 16, 2);
                }
            }
            if (currentRenderTabY == 120) {
                GL11.glPushMatrix();
        ...
        if (isShow && !tier1.knownRecipes().isEmpty()) {
    ...
    public AbstractTrains getRenderEntity() {
        return renderEntity;
    }
}
