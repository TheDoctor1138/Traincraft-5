...
public class GuiJukebox extends GuiScreen {
    ...
    private final int gui_inner_width;
    private final int gui_inner_height;
    private int anim = 0;
    ...
    public void initGui() {
        ...
        int buttonPosX = (this.width - gui_width) / 2;
        int buttonPosY = (this.height - gui_height) / 2;

        if (!jukebox.locked) {
            this.buttonList.add(this.buttonLock = new GuiButton(3, buttonPosX + gui_width - 350, buttonPosY - 10, 51, 10, "Unlocked"));
        } else {
            this.buttonList.add(this.buttonLock = new GuiButton(3, buttonPosX + gui_width - 350, buttonPosY - 10, 43, 10, "Locked"));
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 0) {
            if (streamTextBox.getText() != null && !streamTextBox.getText().isEmpty()) {
                if (!jukebox.isPlaying()) {
    ...
    protected void drawCreativeTabHoveringText(String str, int t, int g) {
        ...
        fontRendererObj.drawStringWithShadow("Current state: " + (jukebox.locked ? "Locked" : "Unlocked"), t + 15, g + 30 - 40, -1);
